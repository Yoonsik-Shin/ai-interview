...
public class LlmGrpcAdapter implements CallLlmPort {
    ...
    private List<InterviewRoleProto> toProtoRoles(List<me.unbrdn.core.interview.domain.enums.InterviewRole> roles) {
        if (roles == null)
            return List.of();
        return roles.stream().map(this::toProtoRole).collect(Collectors.toList());
    }

    private InterviewRoleProto toProtoRole(me.unbrdn.core.interview.domain.enums.InterviewRole role) {
        return switch (role) {
    ...
    private InterviewPersonalityProto toProtoPersonality(
    ...
    private InterviewStageProto toProtoInterviewStage(me.unbrdn.core.interview.domain.enums.InterviewStage stage) {
        if (stage == null) {

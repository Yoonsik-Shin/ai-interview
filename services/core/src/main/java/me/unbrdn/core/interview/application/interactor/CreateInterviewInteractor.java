...
public class CreateInterviewInteractor implements CreateInterviewUseCase {
    ...
    public CreateInterviewResult execute(CreateInterviewCommand command) {
        ...
        if (!(user instanceof Candidate candidate)) {
            throw new UserNotFoundException("사용자가 면접자(Candidate)가 아닙니다. ID: " + command.getUserId());
        }

        // resumeId가 있을 때만 이력서 조회, 없으면 null
        Resumes resume = command.getResumeId()
                .map(resumeId -> loadResumePort.loadById(resumeId)
                        .orElseThrow(() -> new ResumeNotFoundException("이력서를 찾을 수 없습니다. ID: " + resumeId)))
                .orElse(null);

        InterviewType interviewType = command.getType() == null ? InterviewType.PRACTICE : command.getType();

        // Fallback or use provided
        List<InterviewRole> roles = command.getRoles() == null || command.getRoles().isEmpty()
                ? List.of(InterviewRole.TECH)
                : command.getRoles();

        InterviewPersonality personality = command.getPersonality() == null ? InterviewPersonality.COMFORTABLE
                : command.getPersonality();

        String sessionUuid = UUID.randomUUID().toString();

        InterviewSession interviewSession = InterviewSession.create(sessionUuid, candidate, resume, roles, personality,
                interviewType, command.getDomain(), command.getInterviewerCount(), command.getTargetDurationMinutes(),
                command.getSelfIntroduction());

        InterviewSession savedSession = interviewPort.save(interviewSession);

        return CreateInterviewResult.builder().interviewId(savedSession.getId()).status(savedSession.getStatus())
                .build();
    }
}

 ...
 *
 * <p>Output Adapter: Application Layer의 Port를 구현하여 JPA Repository를 래핑합니다.
 *
 * <p>현재는 모놀리식 환경에서 같은 DB의 UsersRepository를 직접 접근합니다. 향후 서버 분리 시에는 LoadUserPort 구현을 별도의
 * UserGrpcAdapter로 분리하여 gRPC 클라이언트로 User 서비스를 호출합니다.
 *
 * <p>서버 분리 전략:
 *
 * <ul>

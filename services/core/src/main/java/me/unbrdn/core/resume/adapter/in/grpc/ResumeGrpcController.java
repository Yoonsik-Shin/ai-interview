...
public class ResumeGrpcController extends ResumeServiceImplBase {
    ...
    public void completeUpload(CompleteUploadRequest request, StreamObserver<CompleteUploadResponse> responseObserver) {
        ...
        try {
            CompleteUploadCommand.CompleteUploadCommandBuilder commandBuilder = CompleteUploadCommand.builder()
    ...
    @Override
    public void validateResume(me.unbrdn.core.grpc.resume.v1.ValidateResumeRequest request,
            io.grpc.stub.StreamObserver<me.unbrdn.core.grpc.resume.v1.ValidateResumeResponse> responseObserver) {
        log.info("gRPC 요청 수신: ValidateResume");
        try {
            me.unbrdn.core.resume.application.dto.ValidateResumeCommand command = me.unbrdn.core.resume.application.dto.ValidateResumeCommand
                    .builder().text(request.getText()).build();

            me.unbrdn.core.resume.application.dto.ValidateResumeResult result = validateResumeUseCase.execute(command);

            me.unbrdn.core.grpc.resume.v1.ValidateResumeResponse response = me.unbrdn.core.grpc.resume.v1.ValidateResumeResponse
                    .newBuilder().setIsResume(result.isResume())
                    .setReason(result.getReason() != null ? result.getReason() : "").setScore(result.getScore())
                    .build();

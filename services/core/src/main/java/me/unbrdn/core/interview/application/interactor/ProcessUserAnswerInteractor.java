...
public class ProcessUserAnswerInteractor implements ProcessUserAnswerUseCase {
    ...
    public void execute(ProcessUserAnswerCommand command) {
        ...
        CallLlmCommand llmCommand = CallLlmCommand.builder().interviewId(command.getInterviewId())
                ...
                .userText(command.getUserText()).availableRoles(interviewSession.getRoles())
    ...
    private void updateTurnCount(String sessionId) {
        try {
            me.unbrdn.core.interview.domain.model.InterviewSessionState state = sessionStatePort.getState(sessionId)
                    .orElseGet(me.unbrdn.core.interview.domain.model.InterviewSessionState::createDefault);
            ...
            log.info("Incremented turn count to {} for session {}", state.getTurnCount(), sessionId);
        } catch (Exception e) {

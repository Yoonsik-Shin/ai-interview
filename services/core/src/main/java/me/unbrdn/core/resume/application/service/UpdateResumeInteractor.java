 ...
 *
 * <p>
 * 비즈니스 로직: 1. 기존 이력서 조회 2. 기존 임베딩 삭제 3. 새 파일로 업데이트 4. 새 임베딩 저장
 */
@Slf4j
@Service
...
public class UpdateResumeInteractor implements UpdateResumeUseCase {
    ...
    public UUID execute(UpdateResumeCommand command) {
        ...
        // 2. 기존 이력서 조회
        Resumes existingResume = loadResumePort.loadResumeById(command.getExistingResumeId())
                .orElseThrow(() -> new ResumeNotFoundException("이력서를 찾을 수 없습니다. ID: " + command.getExistingResumeId()));
        ...
        log.info("이력서 텍스트 추출 완료: resumeId={}, textLength={}", existingResume.getId(), extractedText.length());
        ...
        // 8. 새 임베딩 저장
        resumeVectorService.saveEmbedding(user.getId(), updatedResume.getId().toString(), extractedText, embedding,

...
public class GenerateReportInteractor
        implements CreateInterviewReportUseCase, GetInterviewReportUseCase {
    ...
    public CreateReportResult execute(CreateReportCommand command) {
        ...
        Optional<InterviewReportsJpaEntity> existing =
                reportsRepository.findByInterviewId(interviewId);
        if (existing.isPresent()) {
            InterviewReportsJpaEntity entity = existing.get();
            log.info(
                    "Report already exists for interview {}: status={}",
                    interviewId,
        ...
        // 세션 조회
        InterviewSessionJpaEntity sessionEntity =
                sessionRepository
        ...
        InterviewReportsJpaEntity savedEntity =
    ...
    public void generateReportAsync(UUID interviewId, UUID reportId) {
        log.info(
                "Starting async report generation: interviewId={}, reportId={}",
                interviewId,
                reportId);
        ...
        try {
            List<InterviewMessageJpaEntity> messages =
                    messageJpaRepository.findByInterview_IdOrderByCreatedAtAsc(interviewId);

            GenerateReportResult result =
                    callLlmPort.generateReport(interviewId.toString(), messages);
            ...
            log.info(
                    "Report generation completed: reportId={}, score={}, status={}",
                    reportId,
                    result.getTotalScore(),
                    result.getPassFailStatus());
            ...
            report.fail();
            log.error(
                    "Report generation failed: reportId={}, interviewId={}",
                    reportId,
                    interviewId,
                    e);
        }
    ...
    public void triggerReportGeneration(UUID interviewId) {
        log.info(
                "Triggering report pre-generation at CLOSING_GREETING: interviewId={}",
                interviewId);
        try {
    ...
    public GetReportResult execute(GetReportQuery query) {
        InterviewReportsJpaEntity entity =
                reportsRepository
                        .findById(query.reportId())
                        .orElseThrow(
                                () ->
                                        new IllegalArgumentException(
                                                "Report not found: " + query.reportId()));

        return new GetReportResult(
                entity.getId(),
                entity.getGenerationStatus(),
                entity.getTotalScore() != null ? entity.getTotalScore() : 0,
                entity.getPassFailStatus() != null
                        ? entity.getPassFailStatus()
                        : PassFailStatus.HOLD,
                entity.getSummaryText(),
                entity.getResumeFeedback());
    }
}

...
public class TransitionInterviewStageInteractor implements TransitionInterviewStageUseCase {
    ...
    public void execute(TransitionStageCommand command) {
        InterviewSession session = interviewPort.loadById(command.interviewId())
        ...
        InterviewSessionState state = sessionStatePort.getState(session.getId().toString())
        ...
        default -> {
        } // other stages don't need explicit triggers
        }
        ...
        produceInterviewEventPort.produceMessage(session.getId().toString(), "SYSTEM", "STAGE_TRANSITION",
        ...
        publishTranscriptPort.publish(PublishTranscriptCommand.builder().interviewId(session.getId().toString())
                .type("STAGE_CHANGE").currentStage(command.newStage())
                .previousStage(previousStage != null ? previousStage.name() : "WAITING").build());
    }
    ...
    private void triggerInterviewerIntro(InterviewSession session, InterviewSessionState state) {
        ...
        if (distinctPersonas.size() != participatingPersonas.size()) {
            log.info("Deduplicated interview personas for session {}: original={}, distinct={}", interviewId,
                    participatingPersonas, distinctPersonas);
        }
        ...
        CallLlmCommand llmCommand = CallLlmCommand.builder().interviewId(session.getId().toString())
                .resumeId(session.getResumeId() != null ? session.getResumeId().toString() : null).userId(userId)
                .userText(
                        "면접관님, 지원자에게 간단히 본인 소개를 해주세요. 단, 이름이나 '[면접관 이름]' 같은 임의의 텍스트를 사용하지 말고 (예: 기술 면접관입니다) 본인의 직무 역할만으로 자연스럽게 소개해주세요.")
                .inputRole("system").personaId(firstRoleName).mode(session.getType().name())
                .companyName(session.getCompanyName()).scheduledDurationMinutes(session.getScheduledDurationMinutes())
                .stage(state.getCurrentStage()).domain(session.getDomain()).remainingTimeSeconds(remainingTimeSeconds)
                .currentDifficultyLevel(state.getCurrentDifficulty()).lastInterviewerId(state.getLastInterviewerId())
                .participatingPersonas(state.getParticipatingPersonas()).build();
    ...
    private void triggerFirstQuestion(InterviewSession session, InterviewSessionState state) {
        ...
        CallLlmCommand llmCommand = CallLlmCommand.builder().interviewId(session.getId().toString())
                .resumeId(session.getResumeId() != null ? session.getResumeId().toString() : null).userId(userId)
                .userText("지원자가 자기소개를 마쳤습니다. 이제 이력서를 바탕으로 첫 번째 면접 질문을 시작해주세요.").inputRole("system").personaId("DEFAULT")
                .mode(session.getType().name()).companyName(session.getCompanyName())
                .scheduledDurationMinutes(session.getScheduledDurationMinutes()).stage(state.getCurrentStage())
                .domain(session.getDomain()).remainingTimeSeconds(remainingTimeSeconds)
                .currentDifficultyLevel(state.getCurrentDifficulty()).lastInterviewerId(state.getLastInterviewerId())
                .participatingPersonas(state.getParticipatingPersonas()).build();
    ...
    private void triggerClosingGreeting(InterviewSession session, InterviewSessionState state) {
        ...
        CallLlmCommand llmCommand = CallLlmCommand.builder().interviewId(session.getId().toString())
                .resumeId(session.getResumeId() != null ? session.getResumeId().toString() : null).userId(userId)
                .userText("면접이 모두 끝났습니다. 지원자에게 따뜻한 마무리 인사와 함께 면접 종료를 안내해주세요.").inputRole("system").personaId("DEFAULT")
                .mode(session.getType().name()).companyName(session.getCompanyName())
                .scheduledDurationMinutes(session.getScheduledDurationMinutes()).stage(state.getCurrentStage())
                .domain(session.getDomain()).remainingTimeSeconds(remainingTimeSeconds)
                .currentDifficultyLevel(state.getCurrentDifficulty()).lastInterviewerId(state.getLastInterviewerId())

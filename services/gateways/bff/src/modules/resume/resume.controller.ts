...
import {
    RetryResumeProcessingCommand,
    RetryResumeProcessingUseCase,
} from "./usecases/retry-resume-processing.usecase";
...
export class ResumeController {
    constructor(
        ...
        private readonly retryResumeProcessingUseCase: RetryResumeProcessingUseCase,
    ) {}
    ...
    @ApiOperation({ summary: "FAILED 이력서 재처리 요청" })
    async retryResumeProcessing(@Param("resumeId") resumeId: string) {
        return await this.retryResumeProcessingUseCase.execute(
            new RetryResumeProcessingCommand(resumeId),
        );
    }

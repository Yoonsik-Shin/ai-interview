...
## 2026-02-15: Interview History Query Architecture (Core-first with Future Query Service Migration)

### Context

BFF 서비스가 MongoDB를 직접 조회하여 면접 이력(`GET /v1/interviews/:id/history`)을 반환하고 있었음. 이는 단순 CRUD 프록시에 불과하여 비즈니스 로직(권한 검증, 필터링 등)이 부재하고, MongoDB를 외부에 직접 노출하는 구조적 문제가 있었음.

### Decisions

1. **Short-term: Core Service MongoDB Integration**:
   - Core 서비스에 MongoDB 조회 기능을 추가하여 모든 비즈니스 로직을 중앙화함.
   - BFF는 Core gRPC를 호출하여 이력을 조회하도록 변경.
   - 권한 검증, 데이터 필터링, 변환 로직을 Core UseCase에서 처리.

2. **Long-term: Query Service Separation (CQRS)**:
   - 시스템 성장 시 Query Service(Node.js)를 별도로 분리하여 완전한 CQRS 패턴 구현.
   - **분리 시점 판단 기준**:
     - 조회 트래픽이 Command 트래픽의 10배 이상
     - 조회 응답 시간이 100ms 초과
     - 복잡한 집계/분석 쿼리 필요
     - 다양한 조회 API 증가

3. **Migration Path**:
   - Phase 1: Core에 MongoDB 어댑터 추가 (현재)
   - Phase 2: BFF를 Core gRPC 호출로 전환
   - Phase 3: 트래픽 모니터링 및 성능 측정
   - Phase 4: Query Service 분리 (필요 시)
...
- **Pros (Short-term)**:
  - 비즈니스 로직 중앙화로 일관성 확보.
  - 권한 검증 및 데이터 필터링 강화.
  - 추가 서비스 없이 운영 복잡도 최소화.

- **Cons (Short-term)**:
  - 네트워크 홉 추가로 응답 시간 약간 증가 (예상: +10-30ms).
  - Java에서 MongoDB 조회 (Node.js보다 비효율적).

- **Pros (Long-term)**:
  - Query Service 분리 시 CQRS 패턴의 완전한 이점 확보.
  - 조회 성능 최적화 및 독립적 스케일링 가능.
  - Node.js 기반 MongoDB 조회로 효율성 향상.

- **Cons (Long-term)**:
  - 추가 서비스 운영 및 배포 복잡도 증가.
  - 서비스 간 데이터 동기화 관리 필요.

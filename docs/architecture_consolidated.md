...
- **storage** (Python): Storage Worker (Redis→Object Storage, FastAPI 헬스 체크)
...
**🛡️ Safe Path (안전):** Socket → Redis → Storage Worker (storage) → Object Storage
...
- **storage** (`services/storage`): Redis Consumer → Object Storage Uploader

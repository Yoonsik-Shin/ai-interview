## 2026-02-03 — 13-Step Interview Flow & Sequential multi-interviewer introductions

- **결정**: 기존의 단조로운 면접 흐름을 13단계 프로세스로 확장하고, 특히 **면접관 자기소개(Step 4)** 단계에서 다중 면접관이 순차적으로 발화하도록 **동기식 gRPC 호출(Sync Orchestration)** 방식을 도입한다. 또한 면접 종료 신호 수신 시 즉시 종료하지 않고 **마지막 질문 안내(Step 10)** 및 **마지막 답변(Step 11-12)** 단계를 거치도록 상태 전이 로직을 고도화한다.
- **근거**:
  - **Sequential Intro**: 다중 면접관(2~4인)이 동시에 자기소개를 하면 TTS 오디오가 겹쳐서 들리는 UX 결함 발생. 비동기 스트림 대신 동기식 호출을 통해 앞선 면접관의 발화(TTS)가 끝난 후 다음 면접관이 시작되도록 보장.
  - **Graceful Termination**: LLM이 면접 종료를 결정했을 때 갑자기 연결을 끊는 대신, "준비된 질문이 모두 끝났습니다. 마지막으로 하실 말씀이 있나요?"와 같은 사전 녹음된 가이드를 제공하여 전문적인 면접 경험 제공.
  - **Empathetic Closing**: 마지막 답변 단계에서 LLM이 지원자의 발언에 공감하고 따뜻한 마무리 멘트를 하도록 Prompt Instruction 강화.
- **구현**:
  - **Core (Transition)**: `TransitionInterviewStageInteractor`에서 `generateResponseSync`를 사용하여 면접관별 루프 처리.
  - **Core (Stage)**: `InterviewSession`에 `LAST_QUESTION_PROMPT`, `LAST_ANSWER` 상태 추가 및 `InterviewStage` Enum 동기화.
  - **Frontend (Audio)**: `LAST_QUESTION_PROMPT` 진입 시 `/audio/last_question_prompt_*.mp3` 자동 재생 및 종료 후 `LAST_ANSWER` 전환.
  - **LLM (Prompts)**: `LAST_ANSWER` 단계 전용 `STAGE_INSTRUCTIONS` 추가하여 질문 금지 및 마무리 멘트 강제.
- **트레이드오프**:
  - **성능**: 동기식 gRPC 호출로 인해 모든 면접관의 소개가 끝날 때까지 전체 단계 지연이 발생하지만, "대화의 자연스러움"을 위해 이 지연은 의도된 설계임.
  - **리소스**: 면접관 수만큼 LLM 호출 횟수가 증가하지만, 자기소개는 정해진 텍스트(Prompt)를 기반으로 하므로 토큰 사용량은 미미함.

---

## 2026-02-03 — Interview Session State Management Strategy

- **결정**: `InterviewSession`의 빈번하게 변경되는 상태(`currentDifficulty`, `lastInterviewerId`, `remainingTime`)는 **Redis**에서 관리하고, **RDB**는 세션 생성/종료 및 스테이지 변경 시점에만 업데이트한다. `InterviewSession` 엔티티 자체를 NoSQL로 이관하지 않고 RDB(PostgreSQL/Oracle)에 유지한다.
- **근거**:
  - **RDB 부하 분산**: 턴마다 발생하는 DB 쓰기(Update)를 제거하여 IOPS 절감.
  - **Data Integrity**: User, Resume, Payment 등 타 도메인과의 강한 관계(Foreign Key) 유지를 위해 RDB가 적합.
  - **Redis Atomic**: 동시성 제어가 필요한 상태 값(점수, 난이도)은 Redis의 Atomic Operation이 더 효율적.
  - **NoSQL 역할 구분**: MongoDB(또는 Oracle NoSQL)는 세션 메타데이터가 아니라, 대량의 **대화 로그(Transcript)**와 **이벤트 히스토리(Action Log)** 저장 용도로 제한.
- **구현**:
  - **Redis Key**: `interview:session:{id}:state` (Hash)
    - Field: `difficulty`, `last_interviewer`, `turn_count`
  - **Sync Policy**:
    - **Write-Behind (지연 쓰기)**: 면접 종료 시 Redis 상태를 RDB로 최종 Flush.
    - **Critical Event**: 스테이지 변경(`SELF_INTRO` -> `IN_PROGRESS`) 같은 중요 시점에는 즉시 RDB 동기화.
  - **History**: 난이도 변화 등 추적 데이터는 `InterviewActionHistory` (NoSQL/Log)에 Append-Only로 저장.
- **영향**:
  - Core 서비스의 `ProcessLLMResponse` 로직에서 DB update 제거, Redis update로 변경 필요.
  - RDB 스키마는 유지하되, `current_difficulty` 등은 단순 '스냅샷' 용도로 활용.
...
- **결정**: 프로덕션 환경을 Oracle Kubernetes Engine (OKE)로 구성하며, MinIO를 Oracle Object Storage로, PostgreSQL을 Oracle Autonomous Database (ATP)로 전환한다.
- **근거**:
  - **비용 최적화**: Oracle Cloud Always Free 티어 활용 (ATP 2개, Object Storage 20GB, OKE 노드 6 vCPU)
  - **관리형 서비스**: ATP는 자동 백업, 패치, 고가용성 제공으로 운영 부담 감소
  - **확장성**: Oracle Cloud 인프라로 향후 확장 용이
  - **기존 인프라 재사용**: `k8s/apps/*/prod/` 디렉토리 그대로 사용, 최소한의 수정으로 마이그레이션
- **구현**:
  - **Oracle ATP 연결**:
    - Secret: `oracle-atp-credentials` (username, password, connection-string)
    - ConfigMap: `oracle-atp-config` (datasource-url, jpa-database-platform)
    - Connection Pool 설정: HikariCP 최대 10개 연결 (Always Free 세션 제한 30개 고려)
  - **Oracle Object Storage 연결**:
    - Secret: `oci-object-storage-credentials` (tenancy-id, user-id, fingerprint, private-key, region)
    - ConfigMap: `oci-object-storage-config` (namespace, bucket, endpoint)
  - **리소스 제약 조정**:
    - Core: CPU 500m (1000m → 500m), Memory 1Gi (2Gi → 1Gi)
    - Storage: CPU 500m (1000m → 500m), Memory 512Mi (1Gi → 512Mi)
    - 총 6 vCPU, 24GB RAM 환경에 맞게 조정
  - **배포 스크립트 수정**:
    - `scripts/deploy-prod.sh`: PostgreSQL 제거, Oracle 설정 배포 추가
    - `.github/workflows/deploy.yml`: CI/CD 워크플로우에 Oracle 설정 배포 추가
- **Always Free 제약사항**:
  - **ATP**:
    - 동시 세션: 최대 30개 (Connection Pool 10개로 제한)
    - 자동 정지: 7일간 활동 없으면 자동 정지
    - Private Endpoint 미지원: 퍼블릭 인터넷 mTLS 연결만 가능
  - **Object Storage**:
    - 용량: 총 20GB
    - API 요청: 월 50,000회
  - **OKE**:
    - 총 리소스: 6 vCPU, 24GB RAM (VM.Standard.A1.Flex 2대)
    - 네트워크: 10TB egress/month
- **트레이드오프**:
  - 장점: 무료 티어 활용, 관리형 서비스로 운영 부담 감소, 기존 코드 최소 수정
  - 단점: Always Free 리소스 제약, 세션 제한으로 인한 Connection Pool 조정 필요, Object Storage 용량 제한
- **참고 문서**:
  - `k8s/infra/oracle/prod/README.md`: Oracle 인프라 설정 가이드
  - `archive/docs/oracle-cloud-always-free.md`: OCI Always Free 리소스 상세
